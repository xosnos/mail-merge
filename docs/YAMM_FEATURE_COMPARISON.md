...
*   **Dynamic CC and BCC Support:**
    ...
    *   *Implementation Path:* Update `SendEngine.js` to check for "CC" and "BCC" columns in the sheet and inject those into the `buildMimeMessage` function.
...
*   **In-Sidebar Analytics Dashboard:**
    ...
    *   *Implementation Path:* Update `CardUI.js` to parse the "Merge status" column and display a clean metric summary directly in the Add-on sidebar.
...
*   **Click Tracking:**
    ...
    *   *Implementation Path:* This would require the `SendEngine.js` to find all `href="..."` links in the HTML body and wrap them in a redirect URL pointing to the Central Tracker Web App, which logs the click and then redirects to the original destination.
...
*   **1-Click Unsubscribe Management:**
    ...
    *   *Implementation Path:* Add an "Unsubscribe" endpoint to the Central Tracker Web App. When clicked, it updates a dedicated "Unsubscribes" tab in the Google Sheet. The `SendEngine.js` checks this tab before sending.
...
*   **Personalized Attachments (via Google Drive):**
    ...
    *   *Implementation Path:* Update `SendEngine.js` to look for Drive URLs in an attachment column, fetch the `DriveApp.getFileById()` blobs, and pass them to the `MimeBuilder.js`.
