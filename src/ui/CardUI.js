...
function buildHomepageCard(e) {
  ...
  if (
    e &&
    e.commonEventObject &&
    e.commonEventObject.timeZone &&
    e.commonEventObject.timeZone.offset !== undefined
  ) {
...
function handleSendEmails(e) {
  ...
  if (config.scheduleDate && config.scheduleDate !== '') {
    ...
    if (!isNaN(ms) && ms > now - 60000) {
      ...
      result = {
        success: false,
        message: `Must be future. Selected: ${new Date(ms).toISOString()}, Now: ${new Date(now).toISOString()}, Input: ${config.scheduleDate}`
      };
    }
  } else {
